...
def build_candidates(preset: str) -> List[Candidate]:
    ...
    if preset == "evidence_probe_v1":
        # Experiment 37: locate where evidence fails (per-modality signal vs fusion rules vs arbiter).
        # NOTE: Designed to run efficiently with run_type_arbiter_groups_shared_suite.py
        # using --recompute-consensus-per-candidate, so specialists run once per case.
        base_env = {
            "RCA_COMPONENT_SOURCE": "consensus",
            "RCA_WINDOW_PADDING_MIN": "20",
            "RCA_ENABLE_MODALITY_BONUS": "0",
            "RCA_ENABLE_HINT_BONUS": "0",
            "RCA_ENABLE_HINT_SEED": "0",
            "RCA_COMPONENT_PRIOR_SCALE": "0.0",
            "RCA_USE_JUDGE_MEMORY": "0",
            "RCA_USE_LEGACY_MEMORY": "0",
            "RCA_STRIP_REPLICA_SUFFIX": "0",
            "RCA_ENABLE_REPLICA_EVIDENCE_PREFERENCE": "1",
            "RCA_ENABLE_REPLICA_CANDIDATE_PREFERENCE": "0",
            "RCA_PREFER_REPLICA_HINTS": "0",
            # Baseline windows
            "RCA_ENABLE_BASELINE_WINDOWS": "1",
            "RCA_BASELINE_GAP_MIN": "10",
            "RCA_BASELINE_LEN_MIN": "10",
            # Keep specialist-internal baseline contrast consistent with v9.
            ...
            # Default infra rules ON in baseline; ablations will disable them.
            ...
            # Explicitly keep experimental node boost OFF.
            "RCA_ENABLE_NODE_BOOST": "0",
            "RCA_ENABLE_NODE_STRONG_OVERRIDE": "0",
        }

        def _no_infra_rules_env() -> Dict[str, str]:
            return {
        ...
        def _modality_only_env(*, keep: str) -> Dict[str, str]:
            weights = {
                "RCA_WEIGHT_METRIC": "0",
                "RCA_WEIGHT_LOG": "0",
                "RCA_WEIGHT_TRACE": "0",
            ...
            if keep == "metrics":
                weights["RCA_WEIGHT_METRIC"] = "1.0"
            elif keep == "logs":
                weights["RCA_WEIGHT_LOG"] = "1.0"
            elif keep == "traces":
                weights["RCA_WEIGHT_TRACE"] = "1.0"
            elif keep == "graph":
                weights["RCA_WEIGHT_GRAPH"] = "1.0"

            return {
                **_no_infra_rules_env(),
                # Disable replica preferences to avoid cross-modality coupling.
                "RCA_ENABLE_REPLICA_EVIDENCE_PREFERENCE": "0",
                "RCA_ENABLE_REPLICA_CANDIDATE_PREFERENCE": "0",
                "RCA_PREFER_REPLICA_HINTS": "0",
                **weights,
            }
        ...
        return [
            Candidate(
                key="e0_fusion_baseline",
                name="E0：融合基线（当前v9规则；无type arbiter）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "0",
                },
            ),
            Candidate(
                key="e1_fusion_no_infra_rules",
                name="E1：融合但禁用 infra 规则（无 dominance / 无 gate）",
                env={
                    **base_env,
                    **_no_infra_rules_env(),
                    "RCA_DISABLE_LLM": "1",
            ...
            Candidate(
                key="e2_metric_only",
                name="E2：仅 metrics 证据（权重置零其余模态）",
                env={
                    **base_env,
                    **_modality_only_env(keep="metrics"),
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "0",
                },
            ...
            Candidate(
                key="e3_log_only",
                name="E3：仅 logs 证据（权重置零其余模态）",
                env={
                    **base_env,
                    **_modality_only_env(keep="logs"),
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "0",
                },
            ...
            Candidate(
                key="e4_trace_only",
                name="E4：仅 traces 证据（权重置零其余模态）",
                env={
                    **base_env,
                    **_modality_only_env(keep="traces"),
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "0",
                },
            ...
            Candidate(
                key="e5_graph_only",
                name="E5：仅 graph 证据（权重置零其余模态）",
                env={
                    **base_env,
                    **_modality_only_env(keep="graph"),
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "0",
                },
            ...
            Candidate(
                key="e6_rule_arbiter_postcheck",
                name="E6：规则 type arbiter（postcheck，对照是否“翻走”）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_ENABLE_TYPE_ARBITER": "1",
                    "RCA_TYPE_ARBITER_BACKEND": "rule",
                    "RCA_TYPE_ARBITER_MODE": "postcheck",
                    "RCA_TYPE_ARBITER_CONFIDENCE_THRESHOLD": "0.70",
                    "RCA_TYPE_ARBITER_CLOSE_RATIO": "0.85",
                },
            ),
        ]

    if preset == "kind_aware_rerank_ab_v1":
        # A/B: before vs after kind-aware rerank (two-stage).
        # Designed for ~14 days (phase1 7d + phase2 7d) combined-phase evaluation.
        base_env = {
            "RCA_COMPONENT_SOURCE": "consensus",
            "RCA_WINDOW_PADDING_MIN": "20",
            "RCA_ENABLE_MODALITY_BONUS": "0",
            ...
            "RCA_ENABLE_LOG_BASELINE_CONTRAST": "1",
            "RCA_ENABLE_TRACE_BASELINE_CONTRAST": "1",
            # Infra rules (same as Experiment 37 baseline)
            "RCA_ENABLE_FAULT_LEVEL_GATE": "1",
            "RCA_FAULT_LEVEL_GATE_PENALTY": "0.25",
            "RCA_FAULT_LEVEL_GATE_NODE_MIN_METRIC": "6.0",
            "RCA_FAULT_LEVEL_GATE_NODE_MIN_RATIO": "0.75",
            ...
            "RCA_TIDB_DOMINANCE_MARGIN": "0.02",
            "RCA_TIDB_METRIC_SCORE_MULT": "1.8",
            # A-direction (safer): upweight MetricSpecialist votes for tidb-* to
            # mitigate TiDB under-score against service traces/logs.
            "RCA_WEIGHT_METRIC_TIDB_MULT": "1.0",
            # Keep the more aggressive TiDB competitive floor OFF by default.
            "RCA_ENABLE_TIDB_COMPETITIVE_BOOST": "0",
            # Keep experimental node boost OFF
            "RCA_ENABLE_NODE_BOOST": "0",
            "RCA_ENABLE_NODE_STRONG_OVERRIDE": "0",
            # Ensure determinism
            "RCA_DISABLE_LLM": "1",
        ...
        kind_rerank_env = {
            "RCA_ENABLE_KIND_AWARE_RERANK": "1",
            # In rerank mode, we infer kind and apply penalties; keep old gate enabled in env,
            # but it will be bypassed by code when this flag is on.
            "RCA_KIND_RERANK_PENALTY": "0.30",
            "RCA_KIND_RERANK_NODE_MIN_METRIC": "6.0",
            "RCA_KIND_RERANK_NODE_STRONG_METRIC": "8.0",
            "RCA_KIND_RERANK_NODE_MIN_RATIO": "0.82",
            "RCA_KIND_RERANK_NODE_REQUIRE_TRACES": "0",
            "RCA_KIND_RERANK_TIDB_OVER_NODE_MIN_MARGIN": "0.12",
            "RCA_KIND_RERANK_PENALIZE_NO_EVIDENCE": "1",
            "RCA_KIND_RERANK_NO_EVIDENCE_FACTOR": "0.55",
            "RCA_KIND_RERANK_TIDB_MIN_METRIC": "3.0",
            "RCA_KIND_RERANK_TIDB_MIN_RATIO": "0.72",
            "RCA_KIND_RERANK_BLOCK_ON_REPLICA_LOG": "1",
            "RCA_KIND_RERANK_REPLICA_LOG_BLOCK_RATIO": "0.55",
            "RCA_KIND_RERANK_REPLICA_LOG_BLOCK_SCORE": "2.2",
        }

        return [
            Candidate(
                key="before_baseline",
                name="Before：基线（fault-level gate）",
                env={
                    **base_env,
                },
            ),
            Candidate(
                key="after_kind_aware_rerank",
                name="After：kind-aware rerank（两段式）",
                env={
                    **base_env,
                    **kind_rerank_env,
                },
    ...
    if preset == "kind_aware_rerank_ab_v1_tidb_weight16":
        # Variant of kind_aware_rerank_ab_v1 that enables a safer TiDB under-score fix:
        # upweight MetricSpecialist votes for tidb-* (does not hard-floor scores).
        base_candidates = build_candidates("kind_aware_rerank_ab_v1")
        adjusted: List[Candidate] = []
        for c in base_candidates:
            adjusted.append(
                Candidate(
                    key=c.key,
                    name=c.name + " + TiDB权重1.6",
                    env={
                        **c.env,
                        "RCA_WEIGHT_METRIC_TIDB_MULT": "1.6",
                        "RCA_ENABLE_TIDB_COMPETITIVE_BOOST": "0",
                    },
                )
            )
    ...
    if preset == "tidb_compete_boost_ab_v1":
        # A/B (A direction): before vs after TiDB MetricSpecialist weight multiplier.
        # Goal: mitigate TiDB under-score in a smooth way (no hard score flooring),
        # reducing service->TiDB flips compared to aggressive boosting.
        base_env = {
            "RCA_COMPONENT_SOURCE": "consensus",
            "RCA_WINDOW_PADDING_MIN": "20",
            "RCA_ENABLE_MODALITY_BONUS": "0",
            ...
            "RCA_TIDB_DOMINANCE_MARGIN": "0.02",
            "RCA_TIDB_METRIC_SCORE_MULT": "1.8",
            # Keep aggressive TiDB competitive floor OFF in this preset.
            "RCA_ENABLE_TIDB_COMPETITIVE_BOOST": "0",
            # Keep experimental node boost OFF
            "RCA_ENABLE_NODE_BOOST": "0",
            "RCA_ENABLE_NODE_STRONG_OVERRIDE": "0",
        ...
        return [
            Candidate(
                key="before_no_boost",
                name="Before：TiDB metrics 权重不加成",
                env={
                    **base_env,
                    "RCA_WEIGHT_METRIC_TIDB_MULT": "1.0",
                },
            ...
            Candidate(
                key="after_with_boost",
                name="After：TiDB metrics 权重加成（MetricSpecialist*mult）",
                env={
                    **base_env,
                    **tidb_weight_env,
                },
    ...
    if preset == "type_arbiter_groups_v1":
        base_env = {
            # Keep component selection evidence-driven (consensus), then optionally apply type arbitration.
            "RCA_COMPONENT_SOURCE": "consensus",
            "RCA_WINDOW_PADDING_MIN": "20",
            "RCA_ENABLE_MODALITY_BONUS": "0",
            "RCA_ENABLE_HINT_BONUS": "0",
            ...
            "RCA_TIDB_DOMINANCE_MARGIN": "0.02",
            "RCA_TIDB_METRIC_SCORE_MULT": "1.8",
            # Enable logs/traces baseline contrast (same as v9_baseline_all)
            "RCA_ENABLE_BASELINE_LOGS": "1",
            "RCA_ENABLE_BASELINE_TRACES": "1",
            "RCA_ENABLE_LOG_BASELINE_CONTRAST": "1",
            "RCA_ENABLE_TRACE_BASELINE_CONTRAST": "1",
            # DeepSeek defaults (used only when LLM enabled)
        ...
        return [
            Candidate(
                key="g1_no_llm_no_change",
                name="组1：不调用LLM + 不改算法（当前v9基线）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g2_no_llm_rule_arbiter",
                name="组2：不调用LLM + 规则仲裁减轻混淆（postcheck）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g3_llm_postcheck",
                name="组3：LLM类型仲裁（postcheck）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
                    "RCA_LLM_ATTEMPTS": os.getenv("RCA_LLM_ATTEMPTS", "2"),
                    "RCA_ENABLE_TYPE_ARBITER": "1",
                    "RCA_TYPE_ARBITER_BACKEND": "llm",
                    "RCA_TYPE_ARBITER_MODE": "postcheck",
                    "RCA_TYPE_ARBITER_CONFIDENCE_THRESHOLD": "0.85",
                },
            ),
            Candidate(
                key="g4_llm_pregate",
                name="组4：LLM类型仲裁（pregate软门控重排）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
                    ...
                    "RCA_ENABLE_TYPE_ARBITER": "1",
                    "RCA_TYPE_ARBITER_BACKEND": "llm",
                    "RCA_TYPE_ARBITER_MODE": "pregate",
                    "RCA_TYPE_ARBITER_CONFIDENCE_THRESHOLD": "0.85",
                    "RCA_TYPE_ARBITER_TYPE_BOOST": "0.12",
                    "RCA_TYPE_ARBITER_OTHER_PENALTY": "0.05",
                },
            ),
            Candidate(
                key="g5_llm_hardfilter",
                name="组5：LLM类型仲裁（hardfilter硬过滤）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
                    ...
                    "RCA_ENABLE_TYPE_ARBITER": "1",
                    "RCA_TYPE_ARBITER_BACKEND": "llm",
                    "RCA_TYPE_ARBITER_MODE": "hardfilter",
                    "RCA_TYPE_ARBITER_CONFIDENCE_THRESHOLD": "0.90",
                },
            ),
        ]

    if preset == "type_arbiter_groups_v2_aggressive":
        # Same 5-group setup as v1, but more aggressive node/TiDB dominance to
        # reduce the common GT(node/TiDB) -> Pred(service) failure mode.
        base_env = {
            # Keep component selection evidence-driven (consensus), then optionally apply type arbitration.
            "RCA_COMPONENT_SOURCE": "consensus",
            "RCA_WINDOW_PADDING_MIN": "20",
            ...
            "RCA_BASELINE_GAP_MIN": "10",
            "RCA_BASELINE_LEN_MIN": "10",
            # Fault-level gate (keep same as v1)
            "RCA_ENABLE_FAULT_LEVEL_GATE": "1",
            "RCA_FAULT_LEVEL_GATE_PENALTY": "0.25",
            "RCA_FAULT_LEVEL_GATE_NODE_MIN_METRIC": "6.0",
            "RCA_FAULT_LEVEL_GATE_NODE_MIN_RATIO": "0.75",
            "RCA_FAULT_LEVEL_GATE_TIDB_MIN_METRIC": "3.0",
            "RCA_FAULT_LEVEL_GATE_TIDB_MIN_RATIO": "0.70",
            # Node/TiDB dominance (more aggressive)
            "RCA_ENABLE_NODE_METRIC_DOMINANCE_OVERRIDE": "1",
            "RCA_NODE_DOMINANCE_MIN_METRIC": "4.2",
            "RCA_NODE_DOMINANCE_MIN_RATIO": "0.45",
            "RCA_NODE_DOMINANCE_MARGIN": "0.02",
            "RCA_ENABLE_TIDB_METRIC_DOMINANCE_OVERRIDE": "1",
            "RCA_TIDB_DOMINANCE_MIN_METRIC": "2.0",
            "RCA_TIDB_DOMINANCE_MIN_RATIO": "0.40",
            "RCA_TIDB_DOMINANCE_MARGIN": "0.02",
            "RCA_TIDB_METRIC_SCORE_MULT": "2.2",
            # Enable logs/traces baseline contrast (same as v1)
            "RCA_ENABLE_BASELINE_LOGS": "1",
            "RCA_ENABLE_BASELINE_TRACES": "1",
            "RCA_ENABLE_LOG_BASELINE_CONTRAST": "1",
            "RCA_ENABLE_TRACE_BASELINE_CONTRAST": "1",
        ...
        return [
            Candidate(
                key="g1_no_llm_no_change",
                name="组1：不调用LLM + 不改算法（aggressive dominance）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g2_no_llm_rule_arbiter",
                name="组2：不调用LLM + 规则仲裁（postcheck，aggressive dominance）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "1",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g3_llm_postcheck",
                name="组3：LLM类型仲裁（postcheck，aggressive dominance）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g4_llm_pregate",
                name="组4：LLM类型仲裁（pregate，aggressive dominance）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
            ...
            Candidate(
                key="g5_llm_hardfilter",
                name="组5：LLM类型仲裁（hardfilter，aggressive dominance）",
                env={
                    **base_env,
                    "RCA_DISABLE_LLM": "0",
                    "RCA_REQUIRE_LLM": "0",
