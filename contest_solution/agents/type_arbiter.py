...
class TypeArbiterAgent:
    def __init__(self, client: LLMClient) -> None:
        self.client = client
        self._llm_cache: Dict[str, TypeDecision] = {}

    def enabled(self) -> bool:
        # Default ON after the 5-group comparison (group3: llm + postcheck) performed best.
        # Can be disabled via RCA_ENABLE_TYPE_ARBITER=0.
        return os.getenv("RCA_ENABLE_TYPE_ARBITER", "1") not in {"0", "false", "False"}

    def decide(self, *, uuid: str, query: str, consensus: ConsensusResult) -> TypeDecision:
        backend = (os.getenv("RCA_TYPE_ARBITER_BACKEND", "llm") or "llm").strip().lower()
        if backend == "llm":
    ...
    def _decide_llm(self, *, uuid: str, query: str, consensus: ConsensusResult) -> TypeDecision:
        # Arbiter is an optional enhancement and should fail open.
        # Use a dedicated toggle instead of the global RCA_REQUIRE_LLM.
        require_llm = os.getenv("RCA_TYPE_ARBITER_REQUIRE_LLM", "0") not in {"0", "false", "False"}
        if os.getenv("RCA_DISABLE_LLM", "0") not in {"", "0", "false", "False"}:
            if require_llm:
                raise RuntimeError(
                    "LLM is disabled but RCA_TYPE_ARBITER_REQUIRE_LLM=1 (type arbiter backend=llm)."
                )
        ...
        try:
            attempts = int(os.getenv("RCA_LLM_ATTEMPTS", "2"))
        except ValueError:
            attempts = 2
        ...
        for _ in range(attempts):
            try:
                ...
                last_error = exc
                LOGGER.warning("TypeArbiter LLM call failed: %s", exc)

        if last_error is not None and not raw:
            if require_llm:
                raise RuntimeError(f"TypeArbiter LLM repeatedly failed: {last_error}")
            return TypeDecision(decided_type="unknown", confidence=0.0, raw_response="{}")
        try:
        ...
        decision = TypeDecision(decided_type=t, confidence=conf, raw_response=raw)
